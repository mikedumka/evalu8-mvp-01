-- Migration to fix transfer_player_wave_session logic
...
BEGIN
  ...
  -- Verify target session exists and is in same association
  SELECT association_id INTO v_target_association_id
  FROM sessions
  ...
  IF v_target_association_id != v_association_id THEN
     RAISE EXCEPTION 'Cannot transfer player between sessions of different associations.';
  END IF;
  ...
  -- 3. Check if player is already in target session (to give better error)
  IF EXISTS (SELECT 1 FROM player_sessions WHERE session_id = p_target_session_id AND player_id = p_player_id) THEN
      RAISE EXCEPTION 'Player is already assigned to the target session.';
  END IF;

  -- 4. Perform Update
  UPDATE player_sessions
  SET session_id = p_target_session_id,
      team_number = NULL, -- Reset team assignment
      jersey_number = NULL, -- Reset jersey info
      jersey_color = NULL,
      updated_at = NOW()
  ...
  IF v_rows_updated = 0 THEN
      RAISE EXCEPTION 'Player not found in the current session or could not be moved.';
  END IF;
