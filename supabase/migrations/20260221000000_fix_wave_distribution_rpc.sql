-- Fix distribute_wave_players function to use correct column names and include association_id
...
BEGIN
  ...
  -- 2. Get Cohort ID and Association ID from wave (via first session, assuming consistency)
  SELECT s.cohort_id, s.association_id INTO v_cohort_id, v_association_id
  FROM sessions s
  ...
  FOR i IN 1..v_session_count LOOP
    ...
    BEGIN
        ...
        FOR j IN 1..v_count_for_this_session LOOP
            ...
            -- Fixed: Include association_id and use team_number instead of team_assignment
            INSERT INTO player_sessions (association_id, player_id, session_id, team_number)
            VALUES (v_association_id, v_player_id, v_session_id, v_team_num);
