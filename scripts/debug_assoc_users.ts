...
async function main() {
  ...
  if (!sjamha) {
    const { data: sjamha2 } = await supabase
      .from("associations")
      .select("*")
      .ilike("abbreviation", "SJAMHA");
  ...
  if (membersError) {
    ...
    members.forEach((m) => {
      // @ts-ignore
      console.log(
        ` - ${m.user?.email || m.user_id} [${m.roles.join(", ")}] (Status: ${m.status})`,
      );
    });
  }
  ...
  const evaluators =
    members?.filter(
      (m) => m.roles.includes("Evaluator") || m.roles.includes("evaluator"),
    ) || [];
